...
| 🔒 **本地运行** | 仅监听 `127.0.0.1`，数据不出本机；SSE 长连接保活，**刷新页面不掉服务**（5s 静默期优雅降级，真正关闭页面且超时才退出） |
...
> 💡 **页面即生命周期（优雅降级）**：GUI 通过 SSE 长连接与浏览器"共生"。**刷新页面不会掉服务**——旧连接断开后服务进入 5 秒静默期，新页面加载后自动重连并取消销毁；只有真正关闭页面且超过静默期，服务才会退出。
